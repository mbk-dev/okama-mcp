...
**Render defaults (from spec):** 1500×900 px (`figsize=(10, 6)` @ fixed `dpi=150`).
Every tool accepts optional `width` / `height` pixel parameters (bounds 300–4000) so
the MCP user can request a custom size or aspect ratio; out-of-bounds values raise a
`ValueError` that the `@translates_okama_errors` boundary turns into an actionable
MCP error.
